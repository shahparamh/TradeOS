...
# --- REAL-TIME WEBSOCKET TICK SERVER (0.5s Fluctuations) ---
...
async def websocket_market_endpoint(websocket: WebSocket):
    """Establishes real-time connection and broadcasts mock/live stock ticks every 0.5 seconds."""
    await ws_manager.connect(websocket)
    try:
        while True:
            # Randomly select a watchlisted stock to update
            symbol = random.choice(WATCHLIST)
            try:
                price_data = fetch_live_price(symbol)
                base_price = price_data.get("price", 100.0)
                
                # Dynamic realistic micro-fluctuation (0.01% - 0.05%)
                ...
                pass
            # 0.5 second continuous trading interval!
